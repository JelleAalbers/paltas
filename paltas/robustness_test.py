...
def robustness_test(
    *param_value_pairs,
    ...
    pv_pairs = [
        param_value_pairs[pos : pos + 2] for pos in range(0, len(param_value_pairs), 2)
    ]
    ...
    config = config_header.format(
        CONFIG_PATH=str(config_path.parent), CONFIG_NAME=config_path.name.split(".")[0]
    )
    for param_code, param_value in pv_pairs:
        config += (
    ...
    inf = paltas.Analysis.gaussian_inference.GaussianInference.from_folder(
        dataset_folder
    )
