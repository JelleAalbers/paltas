...
class LOSDG19(LOSBase):
    ...
    def nu_f_nu(nu):
        ...
        nu_f_nu = (
            2 * A * (1 + nu_2 ** (-q)) * np.sqrt(nu_2 / (2 * np.pi)) * np.exp(-nu_2 / 2)
        )
        return nu_f_nu
    ...
    def power_law_dn_dm(self, z, m_min, m_max, n_dm=100):
        ...
        # log quantity
        slope_estimate = 1 / n_dm * np.sum(ldn_dm) * np.sum(lm) - np.sum(ldn_dm * lm)
    ...
    def cone_angle_to_radius(self, z, z_lens, z_source, cone_angle, angle_buffer=0.8):
    ...
    def volume_element(self, z, z_lens, z_source, dz, cone_angle, angle_buffer=0.8):
        ...
        """
        r_los = self.cone_angle_to_radius(
            z + dz / 2, z_lens, z_source, cone_angle, angle_buffer=angle_buffer
        )
    ...
    def convert_to_lenstronomy(self, z, z_masses, z_cart_pos):
        ...
        z_rho_nfw = nfw_functions.rho_nfw_from_m_c(
            z_masses, concentration, self.cosmo, r_scale=z_r_scale
        )
    ...
    def calculate_average_alpha(self, num_pix):
        ...
        for zi, z in enumerate(z_range):
            ...
            # mass.
            pl_slope, pl_norm = self.power_law_dn_dm(z + dz / 2, m_min, m_max)
            m_average = power_law.power_law_integrate(
                m_min, m_max, pl_slope + 1
            ...
            rho_nfw_avg = nfw_functions.rho_nfw_from_m_c(
                m_average, c_average, self.cosmo, r_scale=r_scale_avg
            )
            r_scale_ang_avg, alpha_Rs_avg = nfw_functions.convert_to_lenstronomy_NFW(
                r_scale_avg, z, rho_nfw_avg, z_source, self.cosmo
            )

            # Get our deflection angle and potential for this NFW profile
            ax, ay = NFW().derivatives(x_grid, y_grid, r_scale_ang_avg, alpha_Rs_avg)
            ...
            # arcsecond^2
            dz_in_kpc = (
                self.cosmo.comovingDistance(z - dz / 2, z + dz / 2) / self.cosmo.h
            )
            dz_in_kpc /= 1 + z - dz / 2
            # Mpc to kpc
            dz_in_kpc *= 1000
