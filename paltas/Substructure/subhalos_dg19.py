...
class SubhalosDG19(SubhalosBase):
    ...
    def mass_concentration(self, z, m_200):
        ...
        return dg19_utils.mass_concentration(
            parameter_dict=self.subhalo_parameters, cosmo=self.cosmo, z=z, m_200=m_200
        )
    ...
    def sample_cored_nfw(self, n_subs):
        ...
        host_rho_nfw = nfw_functions.rho_nfw_from_m_c(
            host_m200, host_c, self.cosmo, r_scale=host_r_scale
        )
        ...
        n_accepted_draws = 0
        r_subs = nfw_functions.cored_nfw_draws(
            r_tidal, host_rho_nfw, host_r_scale, r_max, n_subs
        )
        keep_ind, cart_draws = self.rejection_sampling(r_subs, host_r_200, r_3E)

        # Save the cartesian coordinates we want to keep
        cart_pos[n_accepted_draws : n_accepted_draws + np.sum(keep_ind)] = cart_draws[
            keep_ind
        ]
        ...
        while n_accepted_draws < n_subs:
            ...
            # Save the cartesian coordinates we want to keep
            cart_pos[n_accepted_draws : n_accepted_draws + use_keep] = cart_draws[
                keep_ind
            ][:use_keep]
    ...
    def convert_to_lenstronomy(self, subhalo_masses, subhalo_cart_pos):
        ...
        subhalo_z = (
            np.ones(subhalo_masses.shape) * self.main_deflector_parameters["z_lens"]
        )
        ...
        # for lenstronomy.
        sub_r_200 = nfw_functions.r_200_from_m(subhalo_masses, subhalo_z, self.cosmo)
        sub_r_scale = sub_r_200 / concentration
        sub_rho_nfw = nfw_functions.rho_nfw_from_m_c(
            subhalo_masses, concentration, self.cosmo, r_scale=sub_r_scale
        )
        ...
        (
            ...
            sub_r_scale, subhalo_z, sub_rho_nfw, sub_r_trunc, z_source, self.cosmo
        )
        kpc_per_arcsecond = cosmology_utils.kpc_per_arcsecond(z_lens, self.cosmo)
        cart_pos_ang = subhalo_cart_pos / np.expand_dims(kpc_per_arcsecond, axis=-1)
        ...
        for i in range(len(subhalo_masses)):
            ...
            kwargs_list.append(
                {
                    "alpha_Rs": alpha_Rs[i],
                    "Rs": sub_r_scale_ang[i],
                    "center_x": (
                        cart_pos_ang[i, 0] + self.main_deflector_parameters["center_x"]
                    ),
                    "center_y": (
                        cart_pos_ang[i, 1] + self.main_deflector_parameters["center_y"]
                    ),
    ...
    def draw(self, result, **kwargs):
        ...
        result.add_lenses(
            models=model_list, model_kwargs=kwargs_list, redshifts=redshift_list
        )
...
def add_galaxies_in_subhalos(
    ...
    for subhalo_i, (mass, subh, redshift) in enumerate(
        zip(subhalo_masses, subhalo_kwargs, subhalo_redshifts)
    ):
        ...
        sersic_params["R_sersic"] = (
            radian_to_arcsec
            * rhalf_physical_parsec
            / (
        ...
        sersic_params[
            "amp"
        ] = paltas.Sources.sersic.SingleSersicSource.mag_to_amplitude(
            mag_apparent, source_parameters["output_ab_zeropoint"], sersic_params
        ) * ghc_config.get(
    ...
    result.add_lens_light(models=model_list, model_kwargs=kwargs_list)
