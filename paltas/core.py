...
__all__ = ["MagnificationError", "Paltas", "LenstronomyInputs", "BaseComponent"]
...
class Paltas:
    """Main class: parses the configuration files and generates images.
    ...
    def __init__(
        ...
        self.sampler = paltas.Sampling.sampler.Sampler(self.config_dict)
        self.sample = None
        ...
        for model_name in [
            ...
            if model_name in self.config_dict:
                ...
                if model_name == "lens_light":
                    ...
                    model_class = paltas.Sources.source_base.make_lens_light_class(
                        model_class
                    )
        ...
        self.subtract_sky = self.config_dict["detector"]["parameters"].get(
            "subtract_sky", True
        )
    ...
    def get_sample_cosmology(self, as_astropy=False):
        ...
        sample = self.get_current_sample()
        cosmo = paltas.Utils.cosmology_utils.get_cosmology(
    ...
    def _calculate_ps_metadata(
        self, metadata, kwargs_params, point_source_model, lens_model
    ):
        ...
        x_image, y_image = point_source_model.image_position(
            kwargs_params["kwargs_ps"], kwargs_params["kwargs_lens"]
        )
        ...
        if sample["point_source_parameters"]["compute_time_delays"]:
            if "kappa_ext" in sample["point_source_parameters"].keys():
                ...
                if "time_delay_errors" in (sample["point_source_parameters"].keys()):
                    errors = sample["point_source_parameters"]["time_delay_errors"]
                    errors = errors[0 : len(td) - 1]
                    td = td + errors
                td -= td[0]
            else:
                raise ValueError(
                    "must define kappa_ext in point_source "
                    + "parameters to compute time delays"
                )

            # Calculate time delay distance
            metadata[pfix + "ddt"] = paltas.Utils.cosmology_utils.ddt(sample, cosmo)
    ...
    def _draw_image_standard(self, add_noise=True, apply_psf=True, subtract_sky=True):
        ...
        if self.mag_cut is not None:
            ...
            # the image model
            lens_light_total = np.sum(
                image_model.lens_surface_brightness(kwargs_params["kwargs_lens_light"])
            )
            source_light_total = np.sum(
                source_light_model.total_flux(kwargs_params["kwargs_source"])
            )
        ...
        if not subtract_sky:
            ...
            image += (
                single_band._sky_brightness_cps * kwargs_detector["pixel_scale"] ** 2
            )
        ...
        if self.point_source_class is not None:
            self._calculate_ps_metadata(
                metadata, kwargs_params, point_source_model, lens_model
            )
    ...
    def _draw_image_drizzle(self):
        ...
        output_pixel_scale = self.sample["drizzle_parameters"]["output_pixel_scale"]
        ...
        self.sample["detector_parameters"]["pixel_scale"] = supersample_pixel_scale
        ...
        if "supersampling_factor" in self.kwargs_numerics:
            if KWARGSNUMERICWARNING1:
                warnings.warn(
                    "kwargs_numerics supersampling_factor modified " + "for drizzle",
                    category=RuntimeWarning,
                )
                KWARGSNUMERICWARNING1 = False
            self.kwargs_numerics["supersampling_factor"] = max(
                1, int(self.kwargs_numerics["supersampling_factor"] / ss_scaling)
            )
        if "point_source_supersampling_factor" in self.kwargs_numerics:
            if KWARGSNUMERICWARNING2:
                ...
                KWARGSNUMERICWARNING2 = False
            self.kwargs_numerics["point_source_supersampling_factor"] = max(
                1,
                int(
                    self.kwargs_numerics["point_source_supersampling_factor"]
                    / ss_scaling
                ),
            )
        ...
        # noise.
        try:
            image_ss, metadata = self._draw_image_standard(
                add_noise=False, subtract_sky=self.subtract_sky, apply_psf=False
            )
        ...
        if "psf_supersample_factor" in self.sample["drizzle_parameters"]:
            psf_supersample_factor = self.sample["drizzle_parameters"][
                "psf_supersample_factor"
            ]
        else:
            warnings.warn(
                "No psf_supersample_factor provided so 1 will be " + "assumed."
            )
            psf_supersample_factor = 1

        if psf_supersample_factor > ss_scaling:
            raise ValueError(
                f"psf_supersample_factor {psf_supersample_factor} "
        ...
        if self.sample["psf_parameters"]["psf_type"] == "PIXEL" and (
            "point_source_supersampling_factor" not in self.sample["psf_parameters"]
            or self.sample["psf_parameters"]["point_source_supersampling_factor"]
            != psf_supersample_factor
        ):
            raise ValueError(
                "Must specify point_source_supersampling_factor for "
        ...
        self.sample["detector_parameters"]["pixel_scale"] /= psf_supersample_factor
        ...
        single_band = SingleBand(**kwargs_detector)
        data_class = DataAPI(
            numpix=self.numpix * psf_supersample_factor, **kwargs_detector
        ).data_class
        ...
        psf_model = paltas.Utils.lenstronomy_utils.PSFHelper(
            data_class, psf_model_lenstronomy, self.kwargs_numerics
        ).psf_model

        # Pass the high resolution image to hubblify
        image = paltas.Utils.hubble_utils.hubblify(
            image_ss,
            supersample_pixel_scale,
    ...
    def draw_image(self, new_sample=True):
        ...
        try:
            if self.do_drizzle:
                ...
                image, metadata = self._draw_image_standard(
                    add_noise=self.add_noise, subtract_sky=self.subtract_sky
                )
        except MagnificationError:
        ...
        if hasattr(self.config_module, "mask_radius"):
            ...
            x_grid, y_grid = util.make_grid(
                numPix=image.shape[0], deltapix=kwargs_detector["pixel_scale"]
            )
...
class LenstronomyInputs:
    ...
    def __init__(self):
        ...
        self.kwargs_params = dict(
            kwargs_lens=[], kwargs_lens_light=[], kwargs_ps=[], kwargs_source=[]
        )
