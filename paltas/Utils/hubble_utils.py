...
def distort_image(
    img_high_res, w_high_res, w_dither, offset_pattern, psf_supersample_factor
):
    ...
    img_itp = RectBivariateSpline(x_interp, y_interp, img_high_res, kx=1, ky=1)
    ...
    x_dith, y_dith = np.meshgrid(
        np.arange(dith_shape[0]) + 0.5, np.arange(dith_shape[1]) + 0.5, indexing="ij"
    )
    ...
    for oi, offset in enumerate(offset_pattern):
        ...
        ra_off, dec_off = w_offset.all_pix2world(
            x_dith / psf_supersample_factor, y_dith / psf_supersample_factor, WCS_ORIGIN
        )
        x_dith_int, y_dith_int = w_high_res.all_world2pix(ra_off, dec_off, WCS_ORIGIN)
...
def generate_downsampled_wcs(
    ...
    hdul[0].header["CRPIX2"] = (
        low_res_shape[1] / 2,
        "Pixel coordinate of reference point",
    )
    hdul[0].header["CDELT1"] = (
        low_res_pixel_scale / 3600,
        "[deg] Coordinate increment at reference point",
    )
    hdul[0].header["CDELT2"] = (
        low_res_pixel_scale / 3600,
        "[deg] Coordinate increment at reference point",
    )
    hdul[0].header["CUNIT1"] = ("deg", "Units of coordinate increment and value ")
    hdul[0].header["CUNIT2"] = ("deg", "Units of coordinate increment and value ")
    if wcs_distortion:
    ...
    hdul[0].header["CRVAL1"] = (20, "[deg] Coordinate value at reference point")
    hdul[0].header["CRVAL2"] = (-70, "[deg] Coordinate value at reference point")
...
def degrade_image(image, degrade_factor):
    ...
    if image.shape[0] % degrade_factor > 0 or image.shape[1] % degrade_factor > 0:
        raise ValueError("Image dimension is not a multiple degrade_factor")

    # Create the new image to save the output to.
    new_image = np.zeros(
        (image.shape[0] // degrade_factor, image.shape[1] // degrade_factor)
    )
    ...
    for i in range(new_image.shape[0]):
        for j in range(new_image.shape[1]):
            new_image[i, j] = np.sum(
                image[
                    degrade_factor * i : degrade_factor * i + degrade_factor,
...
def upsample_image(image, upsample_factor):
    ...
    # Create the new image to save the output to.
    new_image = np.zeros(
        (image.shape[0] * upsample_factor, image.shape[1] * upsample_factor)
    )
    ...
    for i in range(new_image.shape[0]):
        for j in range(new_image.shape[1]):
            new_image[i, j] = image[
                int(i // upsample_factor), int(j // upsample_factor)
            ]
...
def hubblify(
    ...
    # image.
    w_high_res = generate_downsampled_wcs(
        img_high_res.shape, high_res_pixel_scale, high_res_pixel_scale, None
    )
    w_driz = generate_downsampled_wcs(
        img_high_res.shape, high_res_pixel_scale, drizzle_pixel_scale, None
    )
    w_dither = generate_downsampled_wcs(
        img_high_res.shape, high_res_pixel_scale, detector_pixel_scale, wcs_distortion
    )

    # Initialize our drizzle class with the target output wcs.
    driz = Drizzle(outwcs=w_driz, pixfrac=pixfrac, kernel=kernel, fillval="0")

    # Get the distorted sub images to which we will add noise and then add
    # them to our drizzle.
    img_dither_array = distort_image(
        img_high_res, w_high_res, w_dither, offset_pattern, psf_supersample_factor
    )
    ...
    for d_i, image_dither in enumerate(img_dither_array):
        ...
        driz.add_image(image_dither.T, offset_wcs(w_dither, offset_pattern[d_i]))
