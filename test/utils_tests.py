...
class PowerLawTests(unittest.TestCase):
    ...
    def test_power_law_draw(self):
        ...
        slope = -1.9
        desired_count = 1e2
        norm = desired_count / power_law.power_law_integrate(p_min, p_max, slope)
...
class CosmologyTests(unittest.TestCase):
    ...
    def test_kpc_per_arcsecond(self):
        ...
        for z_test in np.linspace(0.2, 0.8, 20):
            ...
            self.assertAlmostEqual(
                cosmology_utils.kpc_per_arcsecond(z_test, cosmo), dd, places=4
            )
        ...
        np.testing.assert_almost_equal(
            cosmology_utils.kpc_per_arcsecond(z_test, cosmo), dd, decimal=4
        )
    ...
    def test_ddt(self):
        ...
        sample = {
            "main_deflector_parameters": {"z_lens": z_lens},
            "source_parameters": {"z_source": z_source},
        }
        self.assertAlmostEqual(lc.ddt, cosmology_utils.ddt(sample, cosmo), places=2)
    ...
    def test_absolute_to_apparent(self):
        ...
        m_apparent = cosmology_utils.absolute_to_apparent(m_absolute, z_light, cosmo)
        self.assertAlmostEqual(
            m_apparent, m_absolute + 45 + 2.5 * np.log(1 + z_light), places=3
        )

        # Test a closer value
        distance = 1e9 / 1e6 * cosmo.h
        z_light = cosmo.luminosityDistance(distance, inverse=True)
        m_apparent_2 = cosmology_utils.absolute_to_apparent(m_absolute, z_light, cosmo)
        self.assertAlmostEqual(
            m_apparent_2, m_absolute + 40 + 2.5 * np.log(1 + z_light), places=2
        )
    ...
    def test_get_k_correction(self):
        ...
        z_light = 0.5
        self.assertAlmostEqual(
            cosmology_utils.get_k_correction(z_light), 2.5 * np.log(1 + z_light)
        )
        z_light = 0.2134
        self.assertAlmostEqual(
            cosmology_utils.get_k_correction(z_light), 2.5 * np.log(1 + z_light)
...
class HubbleUtilsTests(unittest.TestCase):
    def test_offset_wcs(self):
        ...
        np.testing.assert_almost_equal(
            w.all_pix2world(x, y, 0), w_off.all_pix2world(x, y, 0)
        )
        ...
        x, y = np.meshgrid(np.arange(npix), np.arange(npix))
        np.testing.assert_almost_equal(
            w.all_pix2world(x - 0.5, y, 0), w_off.all_pix2world(x, y, 0)
        )
        ...
        x, y = np.meshgrid(np.arange(npix), np.arange(npix))
        np.testing.assert_almost_equal(
            w.all_pix2world(x - 0.5, y - 0.7, 0), w_off.all_pix2world(x, y, 0)
        )
    ...
    def test_distort_image(self):
        ...
        img_dither_array = hubble_utils.distort_image(
            img_high_res, w_hr, w_lr, offset_pattern, psf_supersample_factor
        )
        ...
        for i in range(len(test_image)):
            for j in range(test_image.shape[1]):
                test_image[i, j] = np.sum(
                    img_high_res[2 * i : 2 * i + 2, 2 * j : 2 * j + 2]
                )
        np.testing.assert_almost_equal(test_image, img_dither_array[0])
        self.assertAlmostEqual(np.sum(img_high_res), np.sum(img_dither_array[0]))
        ...
        for i in range(len(test_image)):
            for j in range(test_image.shape[1]):
                test_image[i, j] = (
                    np.mean(img_high_res[2 * i + 1 : 2 * i + 3, 2 * j : 2 * j + 2]) * 4
                )
        ...
        for i in range(len(test_image)):
            for j in range(test_image.shape[1]):
                test_image[i, j] = (
                    np.mean(img_high_res[2 * i : 2 * i + 2, 2 * j + 1 : 2 * j + 3]) * 4
                )
        ...
        np.testing.assert_almost_equal(img_dither_array[1, :-1, :], img_high_res[1:, :])
        np.testing.assert_almost_equal(img_dither_array[2, :, :-1], img_high_res[:, 1:])
    ...
    def test_generate_downsampled_wcs(self):
        ...
        w_lr = hubble_utils.generate_downsampled_wcs(
            high_res_shape, high_res_pixel_scale, low_res_pixel_scale, wcs_distortion
        )
        w_hr = hubble_utils.generate_downsampled_wcs(
            high_res_shape, high_res_pixel_scale, high_res_pixel_scale, wcs_distortion
        )

        x, y = np.meshgrid(
            np.arange(high_res_shape[0]), np.arange(high_res_shape[1]), indexing="ij"
        )
        np.testing.assert_almost_equal(
            w_lr.all_pix2world(x / 2, y / 2, 1), w_hr.all_pix2world(x, y, 1)
        )

        # Try for another resolution.
        low_res_pixel_scale = 0.03
        w_mr = hubble_utils.generate_downsampled_wcs(
            high_res_shape, high_res_pixel_scale, low_res_pixel_scale, wcs_distortion
        )
        np.testing.assert_almost_equal(
            w_mr.all_pix2world(2 * x / 3, 2 * y / 3, 1),
            w_hr.all_pix2world(x, y, 1),
            decimal=4,
        )
        np.testing.assert_almost_equal(
            w_lr.all_pix2world(3 * x / 4, 3 * y / 4, 1),
            w_mr.all_pix2world(x, y, 1),
            decimal=4,
        )
    ...
    def test_degrade_image(self):
        ...
        np.testing.assert_almost_equal(degrade_2, np.array([[10, 18], [42, 50]]))

        # Now degrade by a factor of 4
        degrade_factor = 4
        degrade_2 = hubble_utils.degrade_image(fake_image, degrade_factor)
        np.testing.assert_almost_equal(degrade_2, np.array([[120]]))
    ...
    def test_hubblify(self):
        ...
        x, y = np.meshgrid(
            np.arange(img_high_res.shape[0]),
            np.arange(img_high_res.shape[1]),
            indexing="ij",
        )
        ...
        img_drizz = hubble_utils.hubblify(
            img_high_res,
        ...
        self.assertAlmostEqual(np.sum(img_drizz), np.sum(img_high_res))
        x, y = np.meshgrid(
            np.arange(img_drizz.shape[0]), np.arange(img_drizz.shape[1]), indexing="ij"
        )
        ...
        for i in range(1, 5):
            self.assertTrue(
                img_drizz[40 * i, 40 * i] > 0 and img_drizz[40 * i, 40 * i] < 20
            )
        ...
        psf_parameters = {"psf_type": "PIXEL", "kernel_point_source": psf_pixel}
        kwargs_detector = {
        ...
        psf_supersample_factor = 2
        psf_pixel[64, :] = np.exp(
            -((np.arange(129) - 64) ** 2) / 100 / psf_supersample_factor**2
        )
        psf_parameters = {"psf_type": "PIXEL", "kernel_point_source": psf_pixel}
        kwargs_detector["pixel_scale"] = detector_pixel_scale / psf_supersample_factor
        psf_model = PSF(**psf_parameters)
        data_class = DataAPI(numpix=numpix, **kwargs_detector).data_class
        psf_helper = lenstronomy_utils.PSFHelper(data_class, psf_model, kwargs_numerics)
        img_drizz_ss = hubble_utils.hubblify(
            img_high_res,
            high_res_pixel_scale,
            detector_pixel_scale,
            drizzle_pixel_scale,
        ...
        np.testing.assert_array_less(
            np.zeros(img_drizz.shape), np.abs(img_drizz - img_drizz_ss)
        )
        np.testing.assert_almost_equal(img_drizz_ss, img_drizz, decimal=2)
