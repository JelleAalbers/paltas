...
class SamplerTests(unittest.TestCase):
        ...
        self.config_dict = {
            ...
            "los": {
                ...
                "parameters": {
                    ...
                    "conc_m_ref": 1e8,
                    "dex_scatter": 0.1,
                    "delta_los": uniform(loc=0, scale=2.0).rvs,
                },
            },
            "main_deflector": {
                "models": None,
                "parameters": {
                    ...
                    "e1,e2": multivariate_normal(
                        np.zeros(2), np.array([[1, 0.5], [0.5, 1]])
                    ).rvs,
            ...
            "source": {
                "class_instance": None,
                "parameters": {"z_source": 1.5, "catalog_i": 200},
            },
            "cosmology": {"parameters": {"cosmology_name": "planck18"}},
            "psf": {"parameters": {"psf_type": "GAUSSIAN", "fwhm": 0.1}},
            "detector": {
                "parameters": {
                    "pixel_scale": 0.08,
                    "ccd_gain": 2.5,
            ...
            "drizzle": {
                "parameters": {
                    "supersample_pixel_scale": 0.020,
                    "output_pixel_scale": 0.030,
                    "wcs_distortion": None,
            ...
            "cross_object": {"parameters": {"los:delta_los,subhalo:sigma_sub": tmn}},
        }
    ...
    def test_sample(self):
        ...
        for _ in range(10):
            ...
            self.assertEqual(
                sample["cosmology_parameters"]["cosmology_name"], "planck18"
            )
            self.assertEqual(sample["source_parameters"]["catalog_i"], 200)
            self.assertGreater(sample["main_deflector_parameters"]["theta_E"], 0)
            self.assertGreater(sample["main_deflector_parameters"]["gamma"], 0)
            self.assertGreater(sample["los_parameters"]["delta_los"], 0)
            self.assertGreater(sample["subhalo_parameters"]["sigma_sub"], 0)
...
class DistributionsTests(unittest.TestCase):
    ...
    def testMultivariateLogNormal(self):
        ...
        np.testing.assert_almost_equal(np.cov(np.log(draws).T), cov, decimal=2)
        np.testing.assert_almost_equal(np.mean(np.log(draws), axis=0), mean, decimal=2)
    ...
    def testTruncatedMultivariateNormal(self):
        ...
        tmn = distributions.TruncatedMultivariateNormal(
            mean, cov, min_values, max_values
        )
        # Check this works
        draw = tmn()
    ...
    def testEllipticitiesTranslation(self):
        ...
        dist = distributions.EllipticitiesTranslation(
            q_dist=uniform(loc=0.0, scale=0.1).rvs,
            phi_dist=uniform(loc=0, scale=np.pi / 4).rvs,
        )
        e1, e2 = dist()
    ...
    def testExternalShearTranslation(self):
        # test mapping w/ constants
        dist = distributions.ExternalShearTranslation(
            gamma_dist=1.5, phi_dist=np.pi / 12
        )
        g1, g2 = dist()
    ...
    def testKappaTransformDistribution(self):
        # test mapping w/ constants
        dist = distributions.KappaTransformDistribution(n_dist=0.2)
        kappa = dist()
        self.assertAlmostEqual(kappa, 1 - 1 / 0.2)
        # test mapping w/ distributions
        dist = distributions.KappaTransformDistribution(
            n_dist=uniform(loc=0.8, scale=0.1).rvs
        )
        kappa = dist()
    ...
    def testDuplicateXY(self):
        ...
        # test mapping w/ distributions
        dist = distributions.DuplicateXY(
            x_dist=uniform(loc=1, scale=1).rvs, y_dist=uniform(loc=-2, scale=1).rvs
        )
        x1, y1, x2, y2 = dist()
    ...
    def testMultipleValues(self):
        # Test size of return
        dist = distributions.MultipleValues(dist=norm(loc=1.0, scale=0.5).rvs, num=5)
        sample = dist()
