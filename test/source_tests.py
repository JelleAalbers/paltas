...
class SourceBaseTests(unittest.TestCase):
    def setUp(self):
        self.c = SourceBase(cosmology_parameters="planck18", source_parameters=dict())
...
class SingleSersicSourceTests(SourceBaseTests):
    def setUp(self):
        ...
        self.c = SingleSersicSource(
            cosmology_parameters="planck18", source_parameters=self.source_parameters
        )
        self.cosmo = get_cosmology("planck18")

    def test_draw_source(self):
        # Check that lenstronomy produces some non-zero image
        light_models, light_kwargs_list, source_redshift_list = self.c.draw_source()
        ...
        image_model = ImageModel(
            data_class=ImageData(
                **data_configure_simple(numPix=n_pixels, deltaPix=pixel_width)
            ),
        ...
        image = image_model.image(
            kwargs_lens=lens_kwargs, kwargs_source=light_kwargs_list
        )
        assert isinstance(image, np.ndarray)
        assert image.sum() > 0

        # Check that the input magnitude was treated as an absolute magnitude
        amp_abs = self.c.mag_to_amplitude(
    ...
    def test_mag_to_amplitude(self):
        ...
        amp_class = self.c.mag_to_amplitude(mag, mag_zeropoint, kwargs_list)
    ...
    def test_get_total_sersic_flux_r(self):
        ...
        def integrated_flux(r, R_sersic, n_sersic, amp_sersic, b_n):
            return (
                ...
                * quad(
                    ...
                    * np.pi
                    * x
                    * np.exp(-b_n * ((x / R_sersic) ** (1 / n_sersic) - 1)),
                    0,
                    r,
                )[0]
            )
        ...
        self.assertAlmostEqual(
            SingleSersicSource.get_total_sersic_flux_r(
                r, R_sersic, n_sersic, amp_sersic
            ),
            integrated_flux(r, R_sersic, n_sersic, amp_sersic, b_n),
        )
        r = R_sersic / 3
        self.assertAlmostEqual(
            SingleSersicSource.get_total_sersic_flux_r(
    ...
    def test_get_total_sersic_flux(self):
        ...
        self.assertAlmostEqual(
            ...
            SingleSersicSource.get_total_sersic_flux(R_sersic, n_sersic, amp_sersic),
        )
...
class DoubleSersicDataTests(SourceBaseTests):
    ...
    def test_get_bulge_disk_half_light(self):
        ...
        flux_R = self.c.get_total_sersic_flux_r(R_total, R_bulge, n_bulge, amp_bulge)
        flux_R += self.c.get_total_sersic_flux_r(R_total, R_disk, n_disk, amp_disk)
    ...
    def test_draw_source(self):
        ...
        self.assertListEqual(light_models, ["SERSIC_ELLIPSE", "SERSIC_ELLIPSE"])
        ...
        self.assertAlmostEqual(
            light_model_bulge.total_flux(light_kwargs_list[:1])[0],
            light_model_disk.total_flux(light_kwargs_list[1:])[0],
        )
...
class GalaxyCatalogTests(SourceBaseTests):
    ...
    def test_draw_source(self):
        ...
        np.testing.assert_almost_equal(np.ones((64, 64)), lens_kwargs[0]["image"])
        ...
        np.testing.assert_almost_equal(
            np.ones((64, 64)) * 10 ** (1 / 2.5), lens_kwargs[0]["image"]
        )
        ...
        mag_apparent = absolute_to_apparent(
            ...
            self.c.source_parameters["z_source"],
            self.c.cosmo,
        )
        ...
        self.assertAlmostEqual(
            mag_apparent,
            cps2magnitude(flux_total, self.c.source_parameters["output_ab_zeropoint"]),
        )
    ...
    def test_k_correct_image(self):
        ...
        mag_k = -2.5 * np.log10(np.sum(image))
        self.assertAlmostEqual(
            mag_orig + get_k_correction(0.8), mag_k + get_k_correction(0.23)
        )
    ...
    def test_normalize_to_mag(self):
        ...
        mag_apparent = -18
        pixel_width = 0.04
        self.c.normalize_to_mag(image, mag_apparent, mag_zeropoint, pixel_width)

        # Use the lens model for full consistency
        light_model = LightModel(["INTERPOL"])
        flux_lenstronomy = light_model.total_flux(
            [
                {
                    "image": image,
                    "center_x": 0.0,
        ...
        self.assertAlmostEqual(
            cps2magnitude(flux_lenstronomy, mag_zeropoint), mag_apparent
        )
    ...
    def test_z_scale_factor(self):
        ...
        z_new = 1.5
        self.assertAlmostEqual(
            self.c.z_scale_factor(z_old, z_new),
...
class COSMOSCatalogTests(SourceBaseTests):
    def setUp(self):
        ...
        self.test_cosmo_folder = (
            os.path.dirname(os.path.abspath(__file__)) + "/test_data/cosmos/"
        )
        self.source_parameters = {
            "smoothing_sigma": 0,
            "max_z": None,
            ...
            "cosmos_folder": self.test_cosmo_folder,
            "random_rotation": False,
    ...
    def test_check_parameterization(self):
        ...
        with self.assertRaises(ValueError):
            COSMOSCatalog(cosmology_parameters="planck18", source_parameters={})

    def test_unfits(self):
        # Check that the returned arrays have the right elements and size.
        rfits = os.path.join(self.test_cosmo_folder, "real_galaxy_catalog_23.5.fits")
        rffits = os.path.join(
            self.test_cosmo_folder, "real_galaxy_catalog_23.5_fits.fits"
        )
        ...
        self.assertTrue(all(elem in rfarray.dtype.names for elem in self.rfkeys))
    ...
    def test_image_and_metadata(self):
        ...
        cs = COSMOSCatalog(cosmology_parameters="planck18", source_parameters=new_sp)
        ...
        image_check = scipy.ndimage.gaussian_filter(
            image, sigma=new_sp["smoothing_sigma"] / HUBBLE_ACS_PIXEL_WIDTH
        )
    ...
    def test_iter_lightmodel_kwargs_samples(self):
        ...
        for lm_list, lm_kwargs in self.c.iter_lightmodel_kwargs_samples(
            ...
            self.assertTrue(
                all(elem in lm_kwargs[0].keys() for elem in lm_keys_required)
            )
    ...
    def test_iter_image_and_metadata_bulk(self):
        ...
        for image, metadata in self.c.iter_image_and_metadata_bulk():
            ...
            self.assertTrue(
                all(
                    elem in metadata.dtype.names
                    for elem in self.rkeys + self.rfkeys[1:]
                )
            )
    ...
    def test_fill_catalog_i_phi_defaults(self):
        ...
        self.assertEqual(
            self.c.fill_catalog_i_phi_defaults(catalog_i=42, phi=42.0), (42, 42.0)
        )

        # Sample random catalog indices otherwise
        results = np.array([self.c.fill_catalog_i_phi_defaults() for _ in range(100)])
    ...
    def test_draw_source(self):
        ...
        np.testing.assert_equal(lm_kwargs["image"], image / lm_kwargs["scale"] ** 2)
        ...
        np.testing.assert_almost_equal(
            lm_kwargs["image"], image_k / metadata["pixel_width"] ** 2
        )
        ...
        image_model = ImageModel(
            data_class=ImageData(
                **data_configure_simple(
                    numPix=n_pixels, deltaPix=metadata["pixel_width"]
                )
            ),
            psf_class=PSF(psf_type="GAUSSIAN", fwhm=0.1 * metadata["pixel_width"]),
            lens_model_class=lens_model,
            source_model_class=light_model,
        )
        # Create a lens that will do nothing
        ...
        l_image = image_model.image(
            kwargs_lens=lens_kwargs, kwargs_source=source_kwargs
        )
...
class COSMOSSersicTests(COSMOSCatalogTests):
    ...
    def test_draw_source(self):
        ...
        ratio_true = 10 ** (-(mag - zeropoint) / 2.5) / 10 ** (
            -(2 * mag - zeropoint) / 2.5
        )
        ...
        complete_image_model = ImageModel(
            data_class=data_class,
            psf_class=psf_class,
            lens_model_class=lens_model,
            source_model_class=light_model,
        )
        im_sersic = complete_image_model.image(
            kwargs_lens=lens_kwargs, kwargs_source=[single_sersic_kwargs]
        )

        # generate COSMOSGalaxy image
        cosmos = COSMOSCatalog(
            cosmology_parameters="planck18", source_parameters=self.source_parameters
        )
        ...
        im_cosmos = complete_image_model.image(
            kwargs_lens=lens_kwargs, kwargs_source=cosmos_kwargs
        )
        ...
        im_cosmossersic = complete_image_model.image(
            kwargs_lens=lens_kwargs, kwargs_source=cosmossersic_kwargs
        )
...
class COSMOSSercicCatalogTests(COSMOSCatalogTests):
    ...
    def test_draw_source(self):
        ...
        self.source_parameters["random_rotation"] = True
        self.c.update_parameters(source_parameters=self.source_parameters)
        _, lm_kwargs_rotated, _ = self.c.draw_source(catalog_i=catalog_i)
