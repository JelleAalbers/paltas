...
class NFWFunctionsTests(unittest.TestCase):
    ...
    def test_cored_nfw_integral(self):
        ...
        analytic_values = nfw_functions.cored_nfw_integral(
            r_tidal, rho_nfw, r_scale, r_upper
        )
    ...
    def test_cored_nfw_draws(self):
        ...
        r_max = 4
        n_subs = int(2e5)
        r_draws = nfw_functions.cored_nfw_draws(
            r_tidal, rho_nfw, r_scale, r_max, n_subs
        )

        n_test_points = 100
        r_test = np.linspace(0, r_max, n_test_points)
        analytic_integral = nfw_functions.cored_nfw_integral(
            r_tidal, rho_nfw, r_scale, r_test
        )

        for i in range(n_test_points):
            self.assertAlmostEqual(
                np.mean(r_draws < r_test[i]),
                analytic_integral[i] / np.max(analytic_integral),
                places=2,
            )
        ...
        for i in range(3):
            self.assertAlmostEqual(
                2 * np.mean(cart_pos[:, i] < -d_max / 4),
                np.mean(cart_pos[:, i] > 0),
                places=1,
            )
    ...
    def test_nfw_integral(self):
        ...
        for i in range(len(r_upper)):
            self.assertAlmostEqual(
                analytic_values[i],
                quad(nfw_func, 1e-20, r_upper[i], args=(rho_nfw, r_scale))[0],
            )
    ...
    def test_nfw_draws(self):
        ...
        r_max = 4
        n_subs = int(2e5)
        r_draws = nfw_functions.nfw_draws(rho_nfw, r_scale, r_max, n_subs)

        n_test_points = 100
        r_test = np.linspace(0, r_max, n_test_points)
        analytic_integral = nfw_functions.nfw_integral(rho_nfw, r_scale, r_test)
    ...
    def test_tnfw_integral(self):
        ...
        r_trunc = 10.54254
        r_upper = np.linspace(0, 4, 100)
        analytic_values = nfw_functions.tnfw_integral(
            rho_nfw, r_scale, r_trunc, r_upper
        )
        ...
        for i in range(len(r_upper)):
            self.assertAlmostEqual(
                analytic_values[i],
                quad(tnfw_func, 1e-20, r_upper[i], args=(rho_nfw, r_scale, r_trunc))[0],
            )
    ...
    def test_tnfw_draws(self):
        ...
        r_max = 50
        n_subs = int(2e5)
        r_draws = nfw_functions.tnfw_draws(rho_nfw, r_scale, r_trunc, r_max, n_subs)

        n_test_points = 1000
        r_test = np.linspace(0, r_max, n_test_points)
        analytic_integral = nfw_functions.tnfw_integral(
            rho_nfw, r_scale, r_trunc, r_test
        )
    ...
    def test_r_200_from_m(self):
        ...
        rhos, rs = profile_nfw.NFWProfile.fundamentalParameters(
            M=m_200 * h, c=c, z=z_lens, mdef="200c"
        )
    ...
    def test_m_c_from_rho_r_scale(self):
        ...
        m_inv, c_inv = nfw_functions.m_c_from_rho_r_scale(
            rho_nfw, r200 / c, cosmo, z_lens
        )
    ...
    def test_calculate_sigma_crit(self):
        ...
        z_lens = 0.2
        z_source = 1.3
        lens_cosmo = LensCosmo(z_lens=z_lens, z_source=z_source, cosmo=cosmo_astropy)
        sigma_crit = nfw_functions.calculate_sigma_crit(z_lens, z_source, cosmo)
        mpc_2_kpc = 1e3
        self.assertAlmostEqual(
            sigma_crit / (lens_cosmo.sigma_crit / mpc_2_kpc**2), 1, places=4
        )
    ...
    def test_convert_to_lenstronomy_NFW(self):
        ...
        for i in range(n_samps):
            lens_cosmo = LensCosmo(
                z_lens=z_halo[i], z_source=z_source, cosmo=cosmo_astropy
            )
            rho0, Rs, r200 = lens_cosmo.nfwParam_physical(M=m_200[i], c=c[i])
            rs_angle_ls, alpha_rs_ls = lens_cosmo.nfw_physical2angle(M=m_200[i], c=c[i])
    ...
    def test_convert_from_lenstronomy_NFW(self):
        ...
        r_scale_inv, rho_nfw_inv = nfw_functions.convert_from_lenstronomy_NFW(
            r_scale_angle, alpha_rs, z_halo, z_source, cosmo
        )
    ...
    def test_convert_to_lenstronomy_tNFW(self):
        ...
        (
            ...
            r_scale, z_lens, rho_nfw, r_trunc, z_source, cosmo
        )
    ...
    def test_convert_from_lenstronomy_tNFW(self):
        ...
        (
            ...
            r_scale, z_halo, rho_nfw, r_trunc, z_source, cosmo
        )
        (
            r_scale_inv,
            rho_nfw_inv,
            r_trunc_inv,
        ) = nfw_functions.convert_from_lenstronomy_tNFW(
            r_scale_angle, alpha_rs, r_trunc_ang, z_halo, z_source, cosmo
        )
...
class SubhalosBaseTests(unittest.TestCase):
    ...
    def test_init(self):
        ...
        self.assertTrue(
            all(
                elem in self.sb.subhalo_parameters.keys()
                for elem in self.subhalo_parameters.keys()
            )
        )
        self.assertTrue(
            all(
                elem in self.sb.main_deflector_parameters.keys()
                for elem in self.main_deflector_parameters.keys()
            )
        )
        self.assertTrue(
            all(
                elem in self.sb.source_parameters.keys()
    ...
    def test_update_parameters(self):
        ...
        self.sb.update_parameters(
            subhalo_parameters=subhalo_parameters,
        ...
        self.assertFalse(
            self.sb.subhalo_parameters["sigma_sub"]
            == self.subhalo_parameters["sigma_sub"]
        )
        self.assertFalse(
            self.sb.main_deflector_parameters["M200"]
            == self.main_deflector_parameters["M200"]
        )
        self.assertFalse(
            self.sb.source_parameters["z_source"] == self.source_parameters["z_source"]
        )
...
class SubhalosDG19Tests(unittest.TestCase):
    ...
    def test_host_scaling_function(self):
        ...
        pre_computed_list = [
            0.0015676639733375364,
            0.014528215260579059,
            0.04576724520325134,
        ]

        for i in range(len(host_m200_list)):
            self.assertAlmostEqual(
                SubhalosDG19.host_scaling_function(host_m200_list[i], z_lens_list[i]),
                pre_computed_list[i],
            )
    ...
    def test_draw_nfw_masses(self):
        ...
        kpc_per_arcsecond = (
            self.cosmo.angularDiameterDistance(self.main_deflector_parameters["z_lens"])
            / self.cosmo.h
            * 1000
            * np.pi
            / 180
        ...
        e_counts = power_law.power_law_integrate(
            subhalo_parameters["m_min"],
            subhalo_parameters["m_max"],
            subhalo_parameters["shmf_plaw_index"],
        )
        norm_without_sigma_sub = (
            ...
            * (
                ...
                ** (-subhalo_parameters["shmf_plaw_index"] - 1)
            )
            * e_counts
        )
        desired_count = 1e2
        self.sd.subhalo_parameters["sigma_sub"] = desired_count / norm_without_sigma_sub
        ...
        for _ in range(n_loops):
            ...
            total_subs += len(masses)
        self.assertEqual(
            np.round(total_subs / n_loops), np.round(desired_count / f_host)
        )
    ...
    def test_mass_concentration(self):
        ...
        np.testing.assert_almost_equal(
            concentrations,
            18 * 1.2 ** (-0.2) * (peak_heights / peak_heights_ref) ** (-0.8),
        )
        ...
        m_200 = np.logspace(6, 10, n_haloes)
        concentrations = self.sd.mass_concentration(z, m_200)
        scatter = np.log10(concentrations) - np.log10(
            18 * 1.2 ** (-0.2) * (peak_heights / peak_heights_ref) ** (-0.8)
        )
        self.assertAlmostEqual(
            np.std(scatter), self.sd.subhalo_parameters["dex_scatter"], places=2
        )
    ...
    def test_rejection_sampling(self):
        ...
        np.testing.assert_almost_equal(np.sqrt(np.sum(cart_pos**2, axis=-1)), r_samps)
        ...
        for i in range(3):
            self.assertAlmostEqual(
                np.max(cart_pos[:, i]) - np.min(cart_pos[:, i]), 2, places=4
            )
        ...
        self.assertEqual(
            np.mean(np.sqrt(cart_pos[:, 0] ** 2 + cart_pos[:, 1] ** 2) < r_3E), 1
        )
    ...
    def test_draw_subhalos(self):
        ...
        for subhalo_kwargs in subhalo_kwargs_list:
            self.assertTrue(
                all(elem in subhalo_kwargs.keys() for elem in required_keys_DG_19)
            )
...
class LOSBaseTests(unittest.TestCase):
    ...
    def test_update_parameters(self):
        ...
        self.assertFalse(
            self.lb.los_parameters["test_1"] == self.los_parameters["test_1"]
        )
        self.assertFalse(
            self.lb.main_deflector_parameters["M200"]
            == self.main_deflector_parameters["M200"]
        )
        self.assertFalse(
            self.lb.source_parameters["z_source"] == self.source_parameters["z_source"]
        )
...
class LOSDG19Tests(unittest.TestCase):
    ...
    def test_nu_f_nu(self):
        ...
        np.testing.assert_almost_equal(modelSheth99(sigma, z), nfn_eval)
    ...
    def test_two_halo_boost(self):
        ...
        for z in np.linspace(0, 1.08, 10):
            self.assertEqual(
                self.ld.two_halo_boost(
                    z, z_lens, dz, lens_m200, r_max, r_min, n_quads=n_quads
                ),
                1,
            )
        ...
        xi_halo *= bias.haloBias(
            lens_m200 * self.cosmo.h, z_lens, mdef="200c", model="tinker10"
        )
        self.assertEqual(
            self.ld.two_halo_boost(
                z, z_lens, dz, lens_m200, r_max, r_min, n_quads=n_quads
    ...
    def test_cone_angle_to_radius(self):
        ...
        for i in range(len(z_first)):
            ...
            r_first[i] = self.ld.cone_angle_to_radius(
                z_first[i], z_lens, z_source, cone_angle
            ) * (1 + z_first[i])
        ...
        np.testing.assert_almost_equal(
            r_first / (1 + z_first),
            cosmology_utils.kpc_per_arcsecond(z_first, self.cosmo) * cone_angle * 0.5,
        )
        ...
        for i in range(len(z_second)):
            # Get the radius in comoving coordinates
            r_second[i] = self.ld.cone_angle_to_radius(
                z_second[i], z_lens, z_source, cone_angle
            ) * (1 + z_second[i])
        ...
        hand_calc = 0.8
        hand_calc *= self.cosmo.comovingDistance(
            z_source
        ...
        hand_calc *= (
            cosmology_utils.kpc_per_arcsecond(z_second, self.cosmo) * cone_angle * 0.5
        )
        ...
        angle_buffer = 1.0
        self.assertAlmostEqual(
            self.ld.cone_angle_to_radius(
                z_source, z_lens, z_source, cone_angle, angle_buffer=angle_buffer
            ),
            0.0,
        )
    ...
    def test_volume_element(self):
        ...
        for i in range(len(z_range)):
            r_z[i] = self.ld.cone_angle_to_radius(
                z_range[i] + dz / 2, z_lens, z_source, cone_angle
            )
            v_z[i] = self.ld.volume_element(
                z_range[i], z_lens, z_source, dz, cone_angle
            )

        dz_in_kpc = (
            self.cosmo.comovingDistance(z_range, z_range + dz)
            / (1 + z_range)
            / self.cosmo.h
            * 1e3
        )
    ...
    def test_draw_nfw_masses(self):
        ...
        pl_slope, pl_norm = self.ld.power_law_dn_dm(
            z, self.los_parameters["m_min"], self.los_parameters["m_max"]
        )
        dV = self.ld.volume_element(
            z,
            self.main_deflector_parameters["z_lens"],
            self.source_parameters["z_source"],
            self.los_parameters["dz"],
        ...
        pl_slope, pl_norm = self.ld.power_law_dn_dm(
            ...
            self.los_parameters["m_min"],
            self.los_parameters["m_max"],
        )
        dV = self.ld.volume_element(
            z,
            self.main_deflector_parameters["z_lens"],
        ...
        halo_boost = self.ld.two_halo_boost(
            z,
            self.main_deflector_parameters["z_lens"],
            self.los_parameters["dz"],
            self.main_deflector_parameters["M200"],
            self.los_parameters["r_max"],
    ...
    def test_sample_los_pos(self):
        ...
        r_los = self.ld.cone_angle_to_radius(
            z + self.los_parameters["dz"] / 2,
            self.main_deflector_parameters["z_lens"],
            self.source_parameters["z_source"],
            self.los_parameters["cone_angle"],
        ...
        cart_pos = cart_pos[where]
        self.assertAlmostEqual(
            2 * np.mean(cart_pos[:, 0] < -d_max / 4),
            np.mean(cart_pos[:, 0] > 0),
            places=2,
        )
        self.assertAlmostEqual(
            2 * np.mean(cart_pos[:, 1] < -d_max / 4),
            np.mean(cart_pos[:, 1] > 0),
            places=2,
        )
    ...
    def test_mass_concentration(self):
        ...
        self.assertAlmostEqual(
            np.std(scatter), self.ld.los_parameters["dex_scatter"], places=2
        )
        self.assertAlmostEqual(np.mean(scatter), 0.0, places=2)
    ...
    def test_draw_los(self):
        ...
        for kwargs in los_kwargs_list:
            self.assertTrue(all(elem in kwargs.keys() for elem in required_keys))
    ...
    def test_calculate_average_alpha(self):
        ...
        for zi in [len(imz) // 2, -1]:
            ...
            self.assertLess(
                np.median(
                    np.abs(
                        np.sqrt(ax_avg**2 + ay_avg**2)
                        - np.sqrt(imax**2 + imay**2)
                    )
                    / np.sqrt(ax_avg**2 + ay_avg**2)
                ),
                0.6,
