...
config_dict = {
    ...
    "los": {
        ...
        "parameters": {
            ...
            "conc_m_ref": 1e8,
            "dex_scatter": 0.1,
            "delta_los": uniform(loc=0, scale=5e-3).rvs,
    ...
    "main_deflector": {
        ...
        "parameters": {
            ...
            "e1,e2": multivariate_normal(
                np.zeros(2), np.array([[0.03, 0.015], [0.015, 0.03]])
            ).rvs,
    ...
    "lens_light": {
        ...
        "parameters": {
            ...
            "n_sersic": truncnorm(-2.0, np.inf, loc=3, scale=0.5).rvs,
            "e1,e2": distributions.EllipticitiesTranslation(
                q_dist=truncnorm(-np.inf, 1.0, loc=0.85, scale=0.15).rvs,
    ...
    "point_source": {
        ...
        "parameters": {
            ...
            "kappa_ext": distributions.KappaTransformDistribution(
                n_dist=norm(loc=1.0, scale=0.025).rvs
            ),
            "time_delay_error": distributions.MultipleValues(
                dist=norm(loc=0.0, scale=0.25).rvs, num=5
            ),
        },
    },
