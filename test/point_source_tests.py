...
class PointSourceBaseTests(unittest.TestCase):
    ...
    def test_update_parameters(self):
        ...
        self.c.update_parameters({"radius": 1.0})
        self.assertDictEqual(self.c.point_source_parameters, {"radius": 1.0})
...
class SinglePointSourceTests(PointSourceBaseTests):
    def setUp(self):
        ...
        self.c = SinglePointSource(point_source_parameters=self.point_source_parameters)
    ...
    def test_check_parameterization(self):
        ...
        failed_parameters = dict(
            x_point_source=0.001, y_point_source=0.001, magnitude=22
        )
    ...
    def test_draw_point_source(self):
        ...
        image_withPS = complete_image_model.image(
            kwargs_lens=lens_kwargs,
            kwargs_source=source_kwargs,
            kwargs_ps=list_ps_kwargs,
        )
        ...
        image_noPS = complete_image_model.image(
            kwargs_lens=lens_kwargs, kwargs_source=source_kwargs, kwargs_ps=None
        )
        ...
        flux_true = magnitude2cps(
            self.c.point_source_parameters["magnitude"],
