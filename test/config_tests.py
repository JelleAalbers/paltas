...
class ConfigUtilsTests(unittest.TestCase):
    ...
    def test_init(self):
        ...
        self.assertTrue(
            isinstance(self.c.los_class, paltas.Substructure.los_dg19.LOSDG19)
        )
        self.assertTrue(
            isinstance(
                self.c.main_deflector_class,
                paltas.MainDeflector.simple_deflectors.PEMDShear,
            )
        )
        self.assertTrue(
            isinstance(
                self.c.subhalo_class, paltas.Substructure.subhalos_dg19.SubhalosDG19
            )
        )
        self.assertTrue(
            isinstance(self.c.source_class, paltas.Sources.cosmos.COSMOSCatalog)
        )
        ...
        self.assertTrue(
            isinstance(
                self.c_all.lens_light_class, paltas.Sources.sersic.SingleSersicSource
            )
        )
        self.assertTrue(
            isinstance(
                self.c_all.point_source_class,
    ...
    def test_get_lenstronomy_models_kwargs(self):
        ...
        self.assertEqual(
            len(kwargs_model["lens_model_list"]), len(kwargs_params["kwargs_lens"])
        )
        self.assertEqual(
            len(kwargs_model["lens_model_list"]),
        ...
        self.assertTrue(
            isinstance(kwargs_params["kwargs_source"][0]["image"], np.ndarray)
        )
        ...
        # changes.
        kwargs_model_new, kwargs_params_new = self.c.get_lenstronomy_models_kwargs(
            new_sample=True
        )
        ...
        self.assertListEqual(kwargs_model["lens_light_model_list"], ["SERSIC_ELLIPSE"])
    ...
    def test_get_metadata(self):
        ...
        # 1 = external shear
        self.assertEqual(
            metadata["main_deflector_parameters_theta_E"],
            kwargs_params["kwargs_lens"][0]["theta_E"],
        )
        self.assertEqual(
            metadata["main_deflector_parameters_gamma1"],
            kwargs_params["kwargs_lens"][1]["gamma1"],
        )
        self.assertEqual(metadata["cosmology_parameters_cosmology_name"], "planck18")
    ...
    def test__calculate_ps_metadata(self):
        ...
        metadata = {}
        self.c_all._calculate_ps_metadata(
    ...
    def test__draw_image_standard(self):
        ...
        self.c.sample["source_parameters"] = {
            "z_source": 0.19499999,
        ...
        self.c.sample["detector_parameters"] = {
            "pixel_scale": orig_meta["pixel_width"],
            "ccd_gain": 2.5,
            "read_noise": 4.0,
        ...
        np.testing.assert_array_less(
            np.ones(image.shape) * 1e-10, np.abs(image - orig_image)
        )

        # Check nothing weird happened to the metadata.
        self.assertListEqual(list(metadata.keys()), list(self.c.get_metadata().keys()))
        ...
        self.c.sample["main_deflector_parameters"] = {
            "M200": 1e13,
            "z_lens": 0.5,
            "gamma": 2.0,
            "theta_E": 0.0,
        ...
        self.c.source_class.k_correct_image(
            orig_image, orig_meta["z"], self.c.sample["source_parameters"]["z_source"]
        )
        self.assertGreater(np.sum(image), np.sum(orig_image))
        self.assertGreater(np.mean(image[0:90, 0:90]), np.mean(image[90:110, 90:110]))
        ...
        self.c.sample["lens_light_parameters"] = {
            "z_source": 0.5,
            "magnitude": 19,
            "output_ab_zeropoint": 25.95,
            "R_sersic": 1.0,
        ...
        self.c.config_dict["lens_light"] = dict(
            parameters=self.c.sample["lens_light_parameters"]
        )
        ...
        self.assertTrue(
            np.sum(lens_light_image[90:110, 90:110]) > np.sum(image[90:110, 90:110])
        )
    ...
    def test__draw_image_drizzle(self):
        ...
        # the source
        c_drizz.sample["source_parameters"] = {
            "z_source": 0.19499999,
            "cosmos_folder": cosmos_folder,
            "max_z": None,
            "minimum_size_in_pixels": None,
        ...
        c_drizz.sample["detector_parameters"] = {
            "pixel_scale": sim_pixel_width,
            "ccd_gain": 1.58,
            "read_noise": 3.0,
            "magnitude_zero_point": 25.127,
            "exposure_time": 1380.0,
            "sky_brightness": 15.83,
            "num_exposures": 1,
            "background_noise": None,
        }
        c_drizz.sample["drizzle_parameters"] = {
            "supersample_pixel_scale": sim_pixel_width,
            "output_pixel_scale": sim_pixel_width,
            "wcs_distortion": None,
        ...
        self.assertEqual(metadata["detector_parameters_pixel_scale"], sim_pixel_width)

        # Make the offset pattern more realistic and change the pixel widths
        c_drizz.sample["drizzle_parameters"]["offset_pattern"] = [
            (0, 0),
            (0.5, 0),
            (0.0, 0.5),
            (0.5, 0.5),
        ...
        c_drizz.sample["main_deflector_parameters"] = {
            "M200": 1e13,
            "z_lens": 0.5,
            "gamma": 2.0,
            "theta_E": 1.0,
        ...
        c_drizz.source_class.k_correct_image(
            orig_image, orig_meta["z"], c_drizz.sample["source_parameters"]["z_source"]
        )
        self.assertGreater(np.sum(image), np.sum(orig_image))
        self.assertGreater(np.mean(image[0:80, 0:80]), np.mean(image[80:90, 80:90]))
        ...
        np.testing.assert_array_less(
            np.ones(image.shape) * 1e-10, np.abs(image - los_image_noise)
        )
    ...
    def test__draw_image_drizzle_psf(self):
        ...
        c_drizz.sample["drizzle_parameters"]["output_pixel_scale"] = sim_pixel_width * 2
        ...
        np.testing.assert_almost_equal(scipy_image, image_degrade_psf, decimal=6)
        ...
        self.assertEqual(
            metadata["detector_parameters_pixel_scale"], sim_pixel_width * 2
        )
        ...
        with self.assertRaises(ValueError):
            c_drizz.sample["psf_parameters"]["point_source_supersampling_factor"] = 4
            c_drizz.sample["psf_parameters"] = {
                "psf_type": "PIXEL",
                "kernel_point_source": psf_pixel,
